...
const AddCourse = () => {
  ...
  const handleChapter = (action, chapterId) => {
    if (action === 'add') {
      ...
      setChapters(chapter.map((chapter) => {
    ...
    justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex flex-col gap-1'>
          ...
          type='number' placeholder='0' min={0} max={100} className='outline-none md:py-2.5
          py-2 w-28 px-3 rounded border border-gray-500' required />
        </div>
                    ...
                    <div className='p-4'>
                      {chapter.chapterContent.map((lecture, lectureIndex) => (
                        <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                          <span className='text-gray-500'>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.
                          lectureDuration} <a href={lecture.lectureUrl}
